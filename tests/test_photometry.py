...
def test_multiple():
    ...
    for i, im in enumerate(images):
        ...
        cm2 = ax[i, 1].imshow(im.data[x[0]:x[-1] + 1, y[0]:y[-1] + 1], origin='lower', vmin=-d_max, vmax=d_max,
                              extent=[y[0] - 0.5, y[-1] + 0.5, x[0] - 0.5, x[-1] + 0.5])
        _ = ax[i, 2].imshow(im.data[x[0]:x[-1] + 1, y[0]:y[-1] + 1] - flux[i] * z, origin='lower', vmin=-d_max,
                            vmax=d_max, extent=[y[0] - 0.5, y[-1] + 0.5, x[0] - 0.5, x[-1] + 0.5])
...
def test_detect():
    ...
    variables = detect.detect_variables_from_difference_image_stack(images, threshold=2.5)

    fig, ax = plt.subplots(len(variables), 3, figsize=(20, len(variables) * 6 + 1))
    ...
    plt.savefig(f"{config_data['output_dir']}/variables.png")
